...
#include <math.h>    // Pour la fonction log
...
// Constantes pour le calcul du thermistor
const float THERMISTOR_NOMINAL = 10000;   // Résistance à 25 degrés Celsius
  ...
  void onWrite(BLECharacteristic *pCharacteristic) {
    ...
    if (rxValue.length() > 0) {
      ...
      if (rxValue.startsWith("SET_MODE")) {
        ...
        if (colon1 != -1) {
          String mode = rxValue.substring(colon1 + 1, colon2 != -1 ? colon2 : rxValue.length());
          String ssid = "";
          String password = "";

          if (colon2 != -1 && colon3 != -1) {
...
void setupServerRoutes() {
  ...
  // Gérer les requêtes non trouvées
  server.onNotFound([](){
  ...
  if (server.hasArg("mode")) {
    ...
    if (connectionMode == "STA") {
      if (server.hasArg("ssid") && server.hasArg("password")) {
        ...
        server.send(200, "application/json", "{\"status\":\"success\",\"message\":\"Bascule vers le mode STA...\"}");
      } else {
        server.send(400, "application/json", "{\"status\":\"error\",\"message\":\"SSID et mot de passe requis pour le mode STA\"}");
      ...
      if (server.hasArg("ap_ssid") && server.hasArg("ap_password")) {
        ...
        server.send(200, "application/json", "{\"status\":\"success\",\"message\":\"Bascule vers le mode AP avec les nouveaux paramètres...\"}");
      } else {
        server.send(400, "application/json", "{\"status\":\"error\",\"message\":\"SSID et mot de passe requis pour le mode AP\"}");
        Serial.println("SSID et mot de passe manquants pour le mode AP");
        connectionMode = ""; // Réinitialiser le mode car les infos sont manquantes
      }
    } else if (connectionMode == "BLE") {
      server.send(200, "application/json", "{\"status\":\"success\",\"message\":\"Bascule vers le mode BLE...\"}");
    } else {
      server.send(400, "application/json", "{\"status\":\"error\",\"message\":\"Mode inconnu\"}");
      Serial.println("Mode inconnu sélectionné");
      connectionMode = ""; // Réinitialiser le mode car invalide
    }
  } else {
    server.send(400, "application/json", "{\"status\":\"error\",\"message\":\"Aucun mode sélectionné\"}");
...
void handleGPIOControl() {
  ...
  if (server.hasArg("action")) {
    ...
    if (action == "on") {
      digitalWrite(LED_PIN, HIGH);
      server.send(200, "application/json", "{\"status\":\"success\",\"message\":\"LED allumée\"}");
      Serial.println("LED allumée");
    } else if (action == "off") {
      digitalWrite(LED_PIN, LOW);
      server.send(200, "application/json", "{\"status\":\"success\",\"message\":\"LED éteinte\"}");
      Serial.println("LED éteinte");
    } else {
      server.send(400, "application/json", "{\"status\":\"error\",\"message\":\"Action invalide. Utilisez 'on' ou 'off'.\"}");
      Serial.println("Action invalide reçue");
    }
  } else {
    server.send(400, "application/json", "{\"status\":\"error\",\"message\":\"Paramètre 'action' manquant.\"}");
...
void handlePlaySong() {
  ...
  server.send(200, "application/json", "{\"status\":\"success\",\"message\":\"Chanson jouée avec succès\"}");
}
...
void handleToggleLedAuto() {
  ...
  if (server.hasArg("state")) {
    ...
    String jsonResponse = "{\"status\":\"success\",\"state\":\"" + String(ledAutoMode ? "on" : "off") + "\"}";
    server.send(200, "application/json", jsonResponse);
  } else {
    server.send(400, "application/json", "{\"status\":\"error\",\"message\":\"Paramètre 'state' manquant\"}");
    Serial.println("Paramètre 'state' manquant");
  }
}
...
void handleGetTemperature() {
  ...
  String jsonResponse = "{\"temperature\": " + String(steinhart, 2) + "}";
  server.sendHeader("Access-Control-Allow-Origin", "*");
  server.send(200, "application/json", jsonResponse);
}

// Fonction pour gérer la page du photoresistor (si vous l'utilisez)
void handlePhotoresistorPage() {
